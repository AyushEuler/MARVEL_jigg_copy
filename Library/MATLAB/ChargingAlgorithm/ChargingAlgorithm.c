...
void ChargingAlgorithm_step(void)
{
  ...
  tmp_7 = !ChargingAlgorithm_U.DataPipeline.VCU.SlowCharge;
  if (tmp_6 || tmp_7) {
  ...
  if (ChargingAlgorithm_U.DataPipeline.VCU.FastCharge && tmp_7) {
  ...
  ChargingAlgorithm_Y.ChargingCurrentReq_mA = (int32_T)floor(fmin(fmin
    (rtb_VoltageDiff_mV, sMultiWord2Double(&tmp_0.chunks[0U], 2, 0)),
