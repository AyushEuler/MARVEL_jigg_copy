...
#define SOC_IN_Recalibrate_SlowCharging ((uint8_T)2U)
...
void SOCEstimation_step(void)
{
   ...
   */
  if (SOCEstimation_U.DataPipeline.VCU.FastChargingSoC) {
    rtb_MinCurrentLimit_mA =
      SOCEstimation_U.CC_Inputs.FastChargingSoC.MinCurrentLimit_mA;
  } else {
    rtb_MinCurrentLimit_mA =
      SOCEstimation_U.CC_Inputs.SlowChargingSoC.MinCurrentLimit_mA;
  }
  ...
  if (SOCEstimation_U.CC_Inputs.LoopTimeSelector) {
    tmp_0 = SOCEstimation_DW.DiscreteTimeIntegrator_DSTATE;
  } else {
    ...
    switch (SOCEstimation_DW.is_c21_SOCEstimation) {
     case SOCEstimation_IN_Full_Charge:
      ...
      if (SOCEstimation_DW.durationCounter_2_g * 100 >=
        ...
        if ((SOCEstimation_U.DataPipeline.isChargeEn != 0) ||
            (SOCEstimation_U.ProtectionOutput_b.OV == Error)) {
     ...
     case SOCEstimation_IN_Full_Discharge:
      ...
      if (SOCEstimation_DW.durationCounter_2_i * 100 >=
        ...
        if ((SOCEstimation_U.DataPipeline.isChargeEn != 1) ||
            (SOCEstimation_U.ProtectionOutput_b.UV == Error)) {
     ...
     case SOCEstimation_IN_Int3:
      ...
      if (SOCEstimation_U.ProtectionOutput_b.OV != Error) {
     ...
     case SOCEstimation_IN_Int4:
      ...
      if (SOCEstimation_U.ProtectionOutput_b.UV != Error) {
  ...
  if (SOCEstimation_U.DataPipeline.VCU.FastChargingSoC) {
    rtb_MaxVoltageLimit_mV =
      SOCEstimation_U.CC_Inputs.FastChargingSoC.MaxVoltageLimit_mV;
    rtb_MaxAllowedImb_mV = SOCEstimation_U.CC_Inputs.FastChargingSoC.MaxAllowedImb_mV;
    rtb_MaxCurrentLimit_mA =
      SOCEstimation_U.CC_Inputs.FastChargingSoC.MaxCurrentLimit_mA;
  } else {
    rtb_MaxVoltageLimit_mV =
      SOCEstimation_U.CC_Inputs.SlowChargingSoC.MaxVoltageLimit_mV;
    rtb_MaxAllowedImb_mV = SOCEstimation_U.CC_Inputs.SlowChargingSoC.MaxAllowedImb_mV;
    rtb_MaxCurrentLimit_mA =
      SOCEstimation_U.CC_Inputs.SlowChargingSoC.MaxCurrentLimit_mA;
  }
  ...
  if (SOCEstimation_DW.is_active_c15_SOCEstimation == 0U) {
    ...
    if (SOCEstimation_U.CC_Inputs.InitialCapacity_Guess) {
      ...
      uint32_T tmp;
      tmp = (uint32_T)SOCEstimation_B.sf_SOC_ReadFromFlash.CapacityRemains_mAh;
      if ((uint32_T)SOCEstimation_B.sf_SOC_ReadFromFlash.CapacityRemains_mAh >
          2147483647U) {
        tmp = 2147483647U;
      }
    ...
    switch (SOCEstimation_DW.is_c15_SOCEstimation) {
     case SOCEstimatio_IN_CoulombCounting:
      ...
      if ((SOCEstimation_DW.durationCounter_1_g * 100 >
          ...
          (SOCEstimation_U.CellBalancingOutput.MaxImbalance_mV <
           rtb_MaxAllowedImb_mV)) {
        SOCEstimation_DW.is_c15_SOCEstimation = SOC_IN_Recalibrate_SlowCharging;
        SOCEstimation_DW.temporalCounter_i1 = 0U;
     ...
     case SOC_IN_Recalibrate_SlowCharging:
      rtb_CC_State = Recalibrate;
   ...
   *  Switch: '<S3>/Switch1'
   */
  rtb_MinCurrentLimit_mA_tmp = (int32_T)floorf(0.000277777785F * tmp_0);
   ...
   *  Product: '<S3>/Divide2'
   */
  rtb_MaxCurrentLimit_mA = (int32_T)floor((real_T)
    SOCEstimation_B.TotalCapacityRemains_mAh / (real_T)
    SOCEstimation_B.sf_UsableCapacity_mAh.MaxUsableCapacity_mAh * 10000.0);

  /* Saturate: '<S1>/Saturation' */
  if (rtb_MaxCurrentLimit_mA > 10000) {
    rtb_MaxCurrentLimit_mA = 10000;
  } else if (rtb_MaxCurrentLimit_mA < 0) {
   ...
   *  DataStoreRead: '<S1>/Data Store Read1'
   */
  rtb_MaxAllowedImb_mV = rtb_MaxCurrentLimit_mA - SOCEstimation_DW.SOC_pct;

  /* Abs: '<S1>/Abs' */
  if (rtb_MaxAllowedImb_mV < 0) {
    rtb_MaxAllowedImb_mV = -rtb_MaxAllowedImb_mV;
  }
  ...
  zcEvent = rt_ZCFcn(RISING_ZERO_CROSSING,
                     ...
                     ((real_T)(rtb_MaxAllowedImb_mV > 1)));
  if (zcEvent != NO_ZCEVENT) {
    /* DataStoreWrite: '<S9>/Data Store Write1' */
    SOCEstimation_DW.SOC_pct = rtb_MaxCurrentLimit_mA;
  }
  ...
  SOCEstimation_Y.CC_Outputs.SOC_cpct = rtb_MaxCurrentLimit_mA;
...
void SOCEstimation_initialize(void)
{
  ...
  SOCEstimation_U.ProtectionOutput_b = SOCEstimation_rtZProtectionOutp;
